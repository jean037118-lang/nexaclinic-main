  ...
  subscribe(listener: Listener) {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          // Atualiza o cache em segundo plano e então notifica os
          // componentes inscritos, em vez de só notificar (que faria
          // os componentes lerem o cache antigo via getAll()).
          fetchAll().then((patients) => {
            _cache = patients;
            notify();
          });
